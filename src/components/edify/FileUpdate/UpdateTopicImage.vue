<template>
  ...
  >
    ...
    <div class="meta">
      <span>Update topic image</span>
    </div>
  </a>
  <VModal
    :open="openUploadFileModal"
    size="large"
    title="Update Topic Image"
    actions="right"
    noclose
...
const onAddTopicImage = (error: any, fileInfo: any) => {
  ...
  if (_file) {
    uploadFormData.topic_image = _file
  }
}
...
const onRemoveTopicImage = (error: any) => {
  ...
  uploadFormData.topic_image = null
}

const upload = async () => {
  isUploadingFile.value = true
  const formData = new FormData()
  formData.append('topic_image', uploadFormData.topic_image)
  try {
    const response = await TopicService.updateTopicImage(props.topic.id, formData)
